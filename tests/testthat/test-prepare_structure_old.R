test_that("prepare source table", {
  dittodb::with_mock_db({
    ...
    expect_equal(dim(df), c(2,3))
    df <- prepare_dimension_levels_table(testthat::test_path("testdata/zadnje_stare/Bilance_proracunov_obcin_1992-2025.xlsx"),
                                          "OB", "OBCINE", con_test)
    expect_equal(dim(df), c(273,3))
    df <- prepare_unit_table(con_test)
    expect_equal(dim(df), c(1,1))
    df <- prepare_series_table(testthat::test_path("testdata/zadnje_stare/Bilance_proracunov_obcin_1992-2025.xlsx"),
                               "OB", "OBCINE", con_test)
